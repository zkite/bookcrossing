def make_shelf(book_resource, book_model, book_schema, book_category_cb, book_owner_cb, book_formation_cb,
               category_model, user_model, user):
    """ Make books shelf and reverse it"""
    book_resource.shelf = list()
    user_books = book_model.query.filter_by(user_id=user.id).all()
    for book in user_books:
        book_resource.shelf.append(
            book_formation_cb(book, book_schema, book_category_cb, book_owner_cb, category_model, user_model))
...
def search_books(pattern, book_model, book_category_cb, book_owner_cb, book_formation_cb, book_schema, category_model,
                 user_model):
    ...
    if serched_books:
        ...
        for book in serched_books:
            formed_book = book_formation_cb(book, book_schema, book_category_cb, book_owner_cb, category_model,
                                            user_model)
...
def get_books_by_category(category_name, category_model, book_model, book_schema, book_owner_cb, book_category_cb,
                          book_formation_cb, user_model):
...
def get_all_books(book_model, book_schema, book_category_cb, book_owner_cb, book_formation_cb, category_model,
                  user_model):
    """ Get all books """
    books = list()
