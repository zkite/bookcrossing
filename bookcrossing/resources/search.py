...
class Search(Resource):
	...
	def get(self):
		...
		books = get_all_books(BookModel, BookSchema(), get_book_category, get_book_owner, book_formation, CategoryModel, UserModel)
		if books:
	...
	def post(self):
		...
		if self.form.validate():
			if search_form:
					books = search_books(search_form, BookModel, get_book_category, get_book_owner, book_formation, BookSchema(), CategoryModel, UserModel)
					if books:
						return make_response(render_template('search.html', books=books, categories=categories), 200, self.headers)
			else:
				return make_response(render_template('search.html', categories=categories), 200, self.headers)

		if select_form:
			books = get_books_by_category(select_form, CategoryModel, BookModel, BookSchema(), get_book_owner, get_book_category, book_formation, UserModel)
			if books:
				return make_response(render_template('search.html', books=books, categories=categories), 200, self.headers)
			else:
				return make_response(render_template('search.html', categories=categories), 200, self.headers)
