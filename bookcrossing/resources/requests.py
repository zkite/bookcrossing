...
class RequestsResource(Resource):
    ...
    def get(self):
        ...
        requests = None
        if select_category:
            requests = get_requests_by_category(select_category, RequestModel, RequestSchema,
                                                BookModel, current_user, UserModel)

        return make_response(render_template('requests.html',
                                             requests=requests,
    ...
    def delete(self, req_id=None):
        ...
        rem_req = remove_request(req_id, db)
        if rem_req:
